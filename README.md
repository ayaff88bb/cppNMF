...
这是可运行、可验证的信号处理工程原型，而不是经过现场认证的状态监测产品。
...
| `.github/` | GitHub 专用配置。目前 `workflows/ci.yml` 定义 Windows 和 Ubuntu 的自动构建与测试。 | 应保留并提交，用于持续验证跨平台构建。 |
...
| `docs/` | 项目的补充文档：架构与算法、验证方法和发布流程。 | 应保留并提交。 |
